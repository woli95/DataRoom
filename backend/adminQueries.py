...
def admin_check_auth(session_token, building_name):
    try:
        connection = create_connection()
        cursor = connection.cursor()
        cursor.execute("SELECT client_id FROM building_client WHERE "
                       "client_id=(SELECT id FROM client WHERE current_session_token='{}') AND "
                       "building_id=(SELECT id FROM building WHERE name='{}');".format(session_token, building_name))
        client_id = cursor.fetchone()[0]
        cursor.execute("SELECT xml_document FROM building WHERE name='{}';".format(building_name))
        xml_doc = cursor.fetchone()[0]
        if 'admin client_id="{}"'.format(client_id) in xml_doc:
            return True
        else:
            return False
    except:
...
def leveladmin_check_auth(session_token, building_name, floor_name):
    try:
        if floor_name is None or building_name is None or session_token is None:
            return False
        connection = create_connection()
        cursor = connection.cursor()
        cursor.execute("SELECT xml_document FROM floor WHERE building_id=(SELECT id FROM building WHERE name='{}') AND name='{}';".format(building_name, floor_name))
        floor_xml = cursor.fetchone()[0]
        cursor.execute("SELECT id FROM client WHERE current_session_token='{}';".format(session_token))
        client_id = cursor.fetchone()[0]
        if 'level_administrator="{}"'.format(client_id) in floor_xml:
            return True
        else:
            return False
    except:
...
def delete_room(building_name, floor_name, room_name):
    try:
        connection = create_connection()
        cursor = connection.cursor()
        ...
        xml_document = myXML.delete_room(xml_document, room_name)
        cursor.execute("UPDATE floor SET xml_document=XMLPARSE(DOCUMENT '{}') WHERE "
                       "name='{}' AND building_id=(SELECT id FROM building WHERE name='{}');".format(xml_document, floor_name, building_name))
        building = storage_client.get_bucket(building_name)
        floor = storage_client.list_blobs(building, prefix=floor_name + '/' + room_name)
        for object in floor:
            object.delete()
        connection.commit()
        return True
